...
**目前正式版本為 v0.10.0**：工作包 018 將解棋改為固定展開的對照工作台，保留大棋盤、本著／曲線／關鍵著／棋譜四分頁與獨立內容捲動，寬橫向仍使用左右雙欄；30 個 test files／227 tests、production build、手機／iPad 直橫式與真實引擎分析生命週期均已驗證。校準資料仍只存目前電腦／瀏覽器，題庫五階也不代表台灣協會級段。
...
npm test           # 目前基準:30 個 test files / 227 tests
