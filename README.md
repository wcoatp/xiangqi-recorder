...
面對面實體象棋對局的**記譜 PWA**:語音/拍照/點棋盤即時記譜、復盤播放與變着編輯、本機引擎解棋、經典殘局題庫與擺盤解析。iPhone / Android / 平板皆可「加入主畫面」安裝,**核心功能完全離線**,不需要任何 API Token。
...
**目前版本為 v0.11.0**：工作包 019 在記譜棋盤加入可即時切換的同向／對向，復盤與解棋加入紅方在下／黑方在下；方向只改畫面與點棋映射，不會改棋譜。31 個 test files／231 tests、production build、兩方向實際點棋、手機／iPad 直橫式與分割視窗均已驗證。校準資料仍只存目前電腦／瀏覽器，題庫五階也不代表台灣協會級段。
...
- **對弈(人機)**:從首頁、漢堡選單或功能指南進入獨立設定頁,再與內建引擎下棋;提供業餘 10 級到業餘 9 段的 19 階相對難度(限制棋力+思考時間調弱;提示/悔棋永遠用全力)。
...
- **開始紀錄**:紅黑方姓名(玩家名冊)、先手選擇、日期時間自動記錄。
  - **座位方向可即時切換**:棋盤旁直接選「同向」或「對向」；對向會把黑方棋子與控制列旋轉 180°，適合把手機／平板平放桌面，雙方看到的都是正向。
  - **三條主要輸入路徑**:
...
- **本機段級校準實驗室（隱藏）**:以正式網址加上 `?rank-calibration=setup`，在目前瀏覽器設定檔啟用後，仍需本機 PIN 才能進入。可建立匿名協助者、自報台灣級／段，選 A01～A10 固定錨點開始現場對弈；App 自動交替紅黑、以固定節點與可重播選著落子，每一著完整保存，可暫離續局並由操作者確認認輸、議和、循環、自然限著或裁判結果。入口不在公開首頁／選單，資料不會自動同步，A01～A10 目前也沒有公開段級映射。
- **復盤紀錄**:自動播放(快/中/慢)、單步、進度拖曳;**變着**(復盤中直接走子即開分支、升為主線)、每著註解、刪除；可隨時切換「紅方在下／黑方在下」，棋子、上一著、分析箭頭與點棋格會一起翻轉。
  ...
  - 復盤紅／黑方在下是目前裝置的 UI 偏好，不在既有備份 v2 五項偏好內；換裝置後可直接重新選擇。
- **解棋**:本機 WASM 引擎逐著分析整局 → 評分曲線、紅黑準確度、**漏着/錯着/敗着**標記、關鍵著法跳轉、每著建議變化。工作台固定展開，上方棋盤維持穩定大尺寸；本著／曲線／關鍵著／棋譜分頁固定可見，長內容只在分析區內捲動，寬橫向則採棋盤／分析左右雙欄。
- **經典殘局題庫**:內建《適情雅趣》12 題五階離線包，可再主動下載 48 題公版精選；可搜尋並依五階難度、來源與本機進度篩選。每題固定提供四個明確入口：
  - **解題練習**：使用者走題目方，本機全力引擎防守；答案不預先顯示，提示需主動開啟。
  - **開始記錄**：局面帶入實體棋盤記譜，紅黑雙方都由真人走。
  - **人機對弈**：局面帶入一般對弈，可選執棋方與 App 相對級／段。
  - **自由分析**：局面帶入三變化分析與試走工作台。
  五階是題庫內部相對難度，未經真人棋手校準，不是協會級段。題包與練習進度只在目前瀏覽器；本輪不隨完整備份搬移。
- **自行擺盤解析**:擺盤編輯器(含棋子數量/位置/將帥對臉合法性檢查)→ 引擎三變化拆解 + 最佳著法箭頭 + 試走跟隨分析。
...
- **殘局資料來源**:首批只散布明代公版古譜《適情雅趣》的局面事實，以及本專案自行整理的 FEN、繁中題名、標籤與五階難度；不收錄現代出版社棋圖、註解、解答或掃描頁。每包保存作品、原題序、館藏連結與權利審查日期。
...
npm test           # 目前基準:31 個 test files / 231 tests
