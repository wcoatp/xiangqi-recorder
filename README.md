...
**目前已驗證 v0.9.0，待正式發布**：工作包 017 新增公版古譜殘局題庫、下載後離線、五階相對難度，以及解題／記錄／人機／分析四入口；30 個 test files／227 tests、production build、離線重開與手機／iPad 響應式驗證已通過。校準資料仍只存目前電腦／瀏覽器，題庫五階也不代表台灣協會級段。
