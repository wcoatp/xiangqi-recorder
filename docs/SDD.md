...
> 文件版本：1.7<br>
> 最後更新：2026-07-17<br>
> 程式基準：`main` / 工作包 007 implementation（本文件同批）<br>
...
| 復盤 | 主線／變著、播放、註解、從任一步另開記譜／對弈局、匯入、匯出、備份 | `src/ui/ReplayPage.tsx`, `src/ui/ContinueFromReplayDialog.tsx`, `src/ui/ImportDialog.tsx` |
...
目前測試基準（2026-07-17）：14 個 test files、101 tests 全部通過。
...
- 單一 App 版本來源與 PWA 三輸入描述（工作包 007，已通過本機驗證，待發布）。

### 下一階段候選

1. 本機段級校準實驗室 Phase 2：humanized policy v1、seed／候選紀錄、校準對弈與匯入去重（工作包 002，需另行核准）。
2. 完整備份格式 v2，納入 allowlist settings、棋子校準與段級校準資料，且排除 PIN／Token。
