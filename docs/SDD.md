...
> 文件版本：1.27<br>
> 最後更新：2026-07-18<br>
> 程式基準：`main` / `5a6ba06` / 工作包 015 Verified（待發布）<br>
...
- `package.json.version` 由 `src/version.ts` 在 build 時封裝，production runtime 不另行讀檔或請求版本資訊。
...
- 完整 `games` 樹與自含接續來源快照；portable stable ID 等於 root node ID。
- 玩家名冊。
...
- 響應式解棋工作台（工作包 015，2026-07-18 已驗證待發布）：直向三段分析面板、寬橫向雙欄與分析分頁，不變更引擎或資料格式；25 個 test files／196 tests 與 320～1366 px 瀏覽器檢查通過。
