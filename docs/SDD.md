...
> 文件版本：1.33<br>
> 最後更新：2026-07-18<br>
> 程式基準：`main` / `11de252` / 工作包 017 Verified（待發布）<br>
...
| D-019 | 解棋採棋盤與分析責任分離的響應式工作台 | 直向以三段固定面板保留棋盤，寬橫向採左右雙欄；分析內容增長與局部捲動不得再參與棋盤尺寸分配。 |
...
| FR-ANALYSIS-001 | 解棋時可穩定對照棋盤與分析結果。 | 同一面板模式下分析前／中／後棋盤尺寸不變；直向可切三段、寬橫向為雙欄，曲線／關鍵著／棋譜皆可同步棋盤。 |
...
目前施工驗證基準（2026-07-18）：30 個 test files、227 tests 全部通過；最近已發布基準仍為工作包 016 的 27 個 test files／216 tests。
...
| 復盤棋盤與分析內容共用單一直向 flex 欄 | 完成分析後曲線、統計與關鍵著可能壓縮棋盤，難以邊看局面邊閱讀。 | 工作包 015 將棋盤／分析分窗，固定直向面板高度並在寬橫向改為雙欄獨立捲動。 |
| v0.7.0 舊頁尚未具備確認式更新 UI | v0.8.0 首次發布時，已開啟的舊 bundle 仍可能依 autoUpdate 自動重載一次。 | v0.8.0 起改用 prompt；真正跨正式版本提示待下一版由既有 v0.8.0 分頁觀察。 |
...
- 經典殘局題庫與局面練習（工作包 017，v0.9.0）：公版古譜 12＋48 題五階題包、主動下載後離線、來源權利帳，以及解題／記錄／人機／分析四入口；30 個 test files／227 tests、production build、離線重開、四入口與 320～1366 px 已驗證，待 commit／push／正式發布。
