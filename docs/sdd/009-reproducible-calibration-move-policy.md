...
> Status：Verified（已由產品負責人於 2026-07-17「繼續完成後續所有施工」授權；待發布）<br>
> Owner：Codex<br>
...
- [ ] 正式站 v0.5.0、COOP／COEP、引擎載入與既有一般流程完成 smoke test。
...
- Commit：未建立。
- Push：未執行。
- Deploy：依 repository 預設在 implementation commit／push 後執行；本機 build 已驗證，尚未部署。
- 正式環境驗證：未執行。
