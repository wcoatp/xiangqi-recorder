...
- [x] Master SDD、SDD 002／010、索引、README 與施工／發布紀錄同步。
- [x] 正式站 v0.7.0、COOP／COEP、資產雜湊、引擎載入與 PIN 入口完成 smoke test。
...
- 實際變更檔案：`src/calibration/matchController.ts` 與測試、`src/store/rankCalibrationMatch.ts` 與 integration tests、`src/ui/RankCalibrationMatch.tsx`、`src/ui/RankCalibrationPage.tsx`、`src/ui/SettingsPage.tsx`、`src/store/backup.integration.test.ts`、`src/styles.css`、package 版本與 SDD 文件。
...
- `npm run build`：通過；104 modules，CSS `index-DvHZeE5n.css`、JS `index-AfR-HGyM.js`。只有既有 tree dynamic/static import 提示與 500 kB chunk size 提示。
...
- Commit：`5b6391c`（`feat: add PIN-gated calibration matches`）；設定入口文案修正 `dac8872`。
- Push：`main` 已推送至 `origin/main`。
- Deploy：2026-07-17 已部署 Firebase Hosting；最終 production build 對應 `dac8872`。
- 正式環境驗證：`https://xiangqi-recorder.web.app/?release=20260717-v070-dac8872` 回傳 HTTP 200、COOP=`same-origin`、COEP=`require-corp`；HTML 載入 `index-AfR-HGyM.js` 與 `index-DvHZeE5n.css`。fresh Chrome 分頁顯示 v0.7.0 與「可開始現場校準對弈」，Fairy-Stockfish xiangqi NNUE 就緒；未設定 PIN 時只顯示門禁設定，Phase 2C 儀表板未暴露。既有 Service Worker 分頁需關閉重開才立即接手新 shell，已以 fresh tab 驗證。
